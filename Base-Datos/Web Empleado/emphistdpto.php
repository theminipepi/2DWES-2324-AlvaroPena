...
try {
    $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        ...
        $stmtEmpleados = $conn->prepare("SELECT empleado.nombre, empleado.dni, emple_depart.fecha_ini, emple_depart.fecha_fin From emple_depart, empleado Where emple_depart.cod_dpto='$cod_dpto' and emple_depart.dni = empleado.dni and emple_depart.fecha_fin is not null");
        $stmtEmpleados->execute();
        $empleados = $stmtEmpleados->fetchAll(PDO::FETCH_ASSOC);
        var_dump($empleados);
        echo "Los empleados que han trabajado en el departamento: ";
        echo "<br>";
        foreach($empleados as $nombre){
            echo $nombre['nombre']." ". $nombre['dni']." ".$nombre['fecha_ini']." ".$nombre['fecha_fin'];
            echo "<br>";
        }
    }
